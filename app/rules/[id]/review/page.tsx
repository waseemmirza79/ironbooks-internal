  ...
  if (!job.ai_completed_at && job.status !== "failed") {
    return (
      <AppShell>
        ...
        />
        <WorkflowStepper currentStep="rules" currentState="active" completedSteps={["coa", "reclass", "revenue"]} clientLinkId={clientLink?.id} />
  ...
  return (
    <AppShell>
      ...
      <WorkflowStepper
        ...
        completedSteps={job.status === "complete" ? ["coa", "reclass", "revenue", "rules"] : ["coa", "reclass", "revenue"]}
